class Game {
  ...
  start() {
    ...
    //4. se agrega la img para el carro 1 y 2
  ...
  play() {
    ...
    if (allPlayers !== undefined) {
      ...
      for (var plr in allPlayers) {
        ...
        //6.guarda el valor de la vida del jugador
        var currentlife = allPlayers[plr].life;

        //7. se crea la condicion para el cambio de imagen
        if (currentlife <= 0) {
          cars[index - 1].changeImage("blast");
          cars[index - 1].scale = 0.3;
        }
        //8. crearemos el metodo handleObstacleCollision
        ...
        if (index === player.index) {
          ...
          //9. se manda a llamar el metodo
          this.handleCarACollisionWithCarB(index);
          this.handleObstacleCollision(index);

          if (player.life <= 0) {
            //14. se cambia a verdadero para la explosion
            this.blast = true;
            //15. se detiene el carro
            this.playerMoving = false;
            //16 cambianos handlelayerControl()
          }
  ...
  handleResetButton() {
    ...
    /******Cambiar los valores para que se vean las barras************* */
  showLife() {
    push();
  ...
  handlePlayerControls() {
    //17. metemos todo dentro del if
    //se activa solo si la this.blast es true.
    if (!this.blast) {
  ...
  //10. se crea un nuevo metodo casi igual que
  //handleObstacleCollision pero ahora colisiona con un carro
  ...
  //11. se comprueba el outpu y se crea una nueva propiedad
  //en el constrictor
